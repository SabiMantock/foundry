...
3. On approval, launch the `orchestrator` subagent with the brief. It will spawn
   `product-analyst` → `architect` (stop for **G1** operator sign-off) → `build-lead` →
   workers → `qa-lead` (G2–G4) → `platform-lead` (G5).
